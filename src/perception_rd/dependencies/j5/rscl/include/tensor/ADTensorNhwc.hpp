 ...
 */
#ifndef _ADMETA_TENSOR_ADTENSOR_NHWC_HPP_
#define _ADMETA_TENSOR_ADTENSOR_NHWC_HPP_

#include "tensor/ADTensor4D.hpp"

namespace ADInference {

class EXPORT_API ADTensorNhwc final : public ADTensor4D {
public:
    ADTensorNhwc(){};
    virtual ~ADTensorNhwc(){};

    ADTensorNhwc(const ShapeVec& shape,
        ...
        : ADTensor4D(shape, TENSOR_SHAPE_MODE_NHWC, mem_type, elem_type, model_adapt) {
    ...
    ADTensorNhwc(const ShapeVec& shape,
                 ...
                 const std::string& mem_type = TENSOR_MEMTYPE_CPU,
                 const bool model_adapt = false)
        : ADTensorNhwc(shape, mem_type, elem_type, model_adapt) {}

    ADTensorNhwc(const void* vir,
                 const void* phy,
                 const ShapeVec& shape,
                 const std::string& mem_type = TENSOR_MEMTYPE_CPU,
                 const PrimitiveType elem_type = PrimitiveType::HPC_FLOAT32,
                 const bool model_adapt = false)
        : ADTensor4D(vir, phy, shape, TENSOR_SHAPE_MODE_NHWC, mem_type, elem_type, model_adapt) {
        this->SetInitDone((Init() == MStatus::M_OK));
    }

    ADTensorNhwc(const DataMgrBasePtr data_mgr,
                 const ShapeVec& shape,
                 const std::string& mem_type = TENSOR_MEMTYPE_CPU,
                 const PrimitiveType elem_type = PrimitiveType::HPC_FLOAT32,
                 const bool model_adapt = false)
        : ADTensor4D(data_mgr, shape, TENSOR_SHAPE_MODE_NHWC, mem_type, elem_type, model_adapt) {
        this->SetInitDone((Init() == MStatus::M_OK));
    }
...
#endif // _ADMETA_TENSOR_ADTENSOR_NHWC_HPP_
