...
#ifndef COMMUNICATION_ACTION_ACTION_SERVICE_IMPL_H_
#define COMMUNICATION_ACTION_ACTION_SERVICE_IMPL_H_
...
class ServiceGoalHandle {
 public:
  ...
  using ActionServ = ActionServiceImpl<SG, SF, SR>;

  using WrapResultMsg = WrappedActionResult<SR>;

  using ResultCode = typename WrappedActionResult<SR>::Code;

  friend ActionServiceImpl<SG, SF, SR>;

 public:
  std::shared_ptr<GoalMsg> GetGoalMsg() const {
    return goal_msg_;
  }

  GoalUUID GetGoalId() const { return uuid_; }

  void PublishFeedback(const std::shared_ptr<FeedbackMsg> &feedback_msg);

  void Succeed(const std::shared_ptr<ResultMsg> &result_msg);

  void Canceled(const std::shared_ptr<ResultMsg> &result_msg);

  void Abort(const std::shared_ptr<ResultMsg> &result_msg);

  bool IsCanceling();

 private:
  ServiceGoalHandle(const std::shared_ptr<ActionServ> &service,
                    const std::shared_ptr<GoalMsg> &goal_msg,
                    const GoalUUID &uuid);
  int WaitResult(std::shared_ptr<WrapResultMsg> &wrap_result_msg);
  bool IsRecvResultReq();
  void RecvCancelGoalReq();

 private:
  std::weak_ptr<ActionServ> weak_service_;
  std::shared_ptr<GoalMsg> goal_msg_;
  GoalUUID uuid_;
  bool is_recv_result_req_;
  bool is_recv_cancel_req_;

  ResultCode result_code_{ResultCode::MAX};
  std::shared_ptr<ResultMsg> result_msg_{nullptr};
...
#ifdef SUPPORT_DDS
      writer_.reset(
          CreateDDSWriter<SF>(comm_attr, feedback_topic_, domain_id, type));
#endif
    } else {
      writer_.reset(
 ...
 private:
  void HandleGoalService(const std::shared_ptr<GoalMsg> &goal_request,
    ...
    if (response.code_ == GoalResponse::Code::REJECT) {
      ...
      auto handle = std::shared_ptr<GoalHandle>(
          new GoalHandle(this->shared_from_this(), goal_request, uuid));
  ...
  void HandleResultService(
    ...
    #endif
    COHLOGD("HandleResultService received result request 2, uuid: {}",
            result_request->uuid_.val_);
  ...
  void PublishFeedback(GoalHandle *handle,
    ...
    if (PROTOCOL_DDS == type_) {
      ...
      DDSSequenceId relative_seq_id;
      relative_seq_id.low = static_cast<uint32_t>(handle->GetGoalId().val_);
      relative_seq_id.high = handle->GetGoalId().val_ >> 32;
      sp_msg->SetRelativeSeqId(relative_seq_id);
...
#endif
    auto sp_msg = std::make_shared<MessageInfo>(0, 0, handle->GetGoalId().val_);
    if (ErrorCode::COMM_CODE_OK != writer_->Write(feedback_msg, sp_msg)) {
      COHLOGE("writer feedback_msg  failed");
    }
  }
...
// ServiceGoalHandle definition
template<typename SG, typename SF, typename SR>
ServiceGoalHandle<SG, SF, SR>::ServiceGoalHandle(
    const std::shared_ptr<ActionServ> &service,
    const std::shared_ptr<GoalMsg> &goal_msg, const GoalUUID &uuid)
    : weak_service_(service),
      goal_msg_(goal_msg),
      uuid_(uuid),
      is_recv_result_req_(false),
      is_recv_cancel_req_(false) {}

template<typename SG, typename SF, typename SR>
void ServiceGoalHandle<SG, SF, SR>::PublishFeedback(
    const std::shared_ptr<FeedbackMsg> &feedback_msg) {
  std::shared_ptr<ActionServ> shared_service = weak_service_.lock();
  if (shared_service) {
    shared_service->PublishFeedback(this, feedback_msg);
  }
}

template<typename SG, typename SF, typename SR>
void ServiceGoalHandle<SG, SF, SR>::Succeed(
    const std::shared_ptr<ResultMsg> &result_msg) {
  {
    std::lock_guard<std::mutex> lk{mutex_};
    result_msg_ = result_msg;
    result_code_ = ResultCode::SUCCEEDED;
  }
  cv_.notify_all();
}

template<typename SG, typename SF, typename SR>
void ServiceGoalHandle<SG, SF, SR>::Canceled(
    const std::shared_ptr<ResultMsg> &result_msg) {
  {
    std::lock_guard<std::mutex> lk{mutex_};
    result_msg_ = result_msg;
    result_code_ = ResultCode::CANCELED;
  }
  cv_.notify_all();
}

template<typename SG, typename SF, typename SR>
void ServiceGoalHandle<SG, SF, SR>::Abort(
    const std::shared_ptr<ResultMsg> &result_msg) {
  {
    std::lock_guard<std::mutex> lk{mutex_};
    result_msg_ = result_msg;
    result_code_ = ResultCode::ABORTED;
  }
  cv_.notify_all();
}

template<typename SG, typename SF, typename SR>
bool ServiceGoalHandle<SG, SF, SR>::IsCanceling() {
  return is_recv_cancel_req_;
}

template<typename SG, typename SF, typename SR>
int ServiceGoalHandle<SG, SF, SR>::WaitResult(
    std::shared_ptr<WrapResultMsg> &wrap_result_msg) {
  std::unique_lock<std::mutex> lk{mutex_};
  if (result_code_ == ResultCode::MAX) {
    cv_.wait(lk, [this] { return result_msg_.get() != nullptr; });
  }
  // COHLOGD("result_code_ is {}!", result_code_);
  wrap_result_msg.reset(new WrapResultMsg(result_code_, result_msg_));
#ifdef SUPPORT_DDS
  wrap_result_msg->ConvertToDDSUnderlayResult();
  #endif
  return 0;
}

template<typename SG, typename SF, typename SR>
bool ServiceGoalHandle<SG, SF, SR>::IsRecvResultReq() {
  return is_recv_result_req_;
}

template<typename SG, typename SF, typename SR>
void ServiceGoalHandle<SG, SF, SR>::RecvCancelGoalReq() {
  is_recv_cancel_req_ = true;
}

}  // namespace communication
}  // namespace hobot

#endif  // COMMUNICATION_ACTION_ACTION_SERVICE_IMPL_H_
